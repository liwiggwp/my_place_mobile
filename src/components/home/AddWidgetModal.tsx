...
export const AddWidgetModal: React.FC<AddWidgetModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/50 backdrop-blur-sm animate-fade-in p-0 sm:p-4">
      <div className="w-full max-w-md bg-white rounded-t-3xl sm:rounded-3xl p-5 sm:p-6 shadow-2xl border border-slate-200 max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
            ...
            <div className={`grid ${availableSizes.length === 2 ? 'grid-cols-2' : 'grid-cols-3'} gap-2`}>
              {availableSizes.map(size => {
                ...
                return (
                  ...
                  >
                    {size === 'small' && (selectedType === 'photo' ? 'S (1x1 Квадрат)' : 'S (Маленький)')}
                    {size === 'medium' && 'M (Средний)'}
                    {size === 'large' && (selectedType === 'photo' ? 'L (Большой постер)' : 'L (Большой)')}
                  </button>
                );
              })}
            </div>
